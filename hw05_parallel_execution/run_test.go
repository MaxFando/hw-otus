...
func TestRun(t *testing.T) {
	...
	t.Run("if were errors in first M tasks, than finished not more N+M tasks", func(t *testing.T) {
		...
		sumTasksCount := int32(workersCount + maxErrorsCount)

		require.Truef(t, errors.Is(err, ErrErrorsLimitExceeded), "actual err - %v", err)
		require.LessOrEqual(t, atomic.LoadInt32(&runTasksCount), sumTasksCount, "extra tasks were started")
	})
	...
	t.Run("tasks without errors Eventually", func(t *testing.T) {
		...
		}, time.Second, time.Millisecond, "Не все задачи были выполнены")
	})
}
